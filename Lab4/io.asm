...
# First, read and echo 3 lines of input by polling the IO registers, not through interrupts:
	...
	decr:
		...
		bne $s0,$zero,l1		# If not zero, get another line
...
# Interrupt handler. Replaces the standard SPIM handler.
	...
	bne $t5,$zero,exception		# Not interrupt, it is exception
...
# Receiver interrupt:
	...
	check_trans:
		beq $t7, $zero ret_handler	# No char to write yet
		and $t5 $t4 0x400	# Check for IP2
		beq $t5, $zero check_loop
...
# Transmitter interrupt:
	...
	check_loop:
		bne $s0, $zero, ret_handler	# If line counter not zero, get another line
